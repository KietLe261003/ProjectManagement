...
export function DetailProject({ project, isOpen, onClose }: DetailProjectProps) {
  ...
  const { call: runDocMethodCall } = useFrappePostCall('frappe.client.run_doc_method');
  
  // Fetch project owner details
  const { data: ownerData } = useFrappeGetDoc(
    "User",
    project?.owner || "",
    project?.owner ? "User" : undefined
  );
  ...
  const handleEditOwner = async (data: OwnerFormData) => {
    ...
    try {
      ...
      try {
        ...
        console.log('Owner changed successfully using save method');
      } catch (saveError) {
        ...
        try {
          ...
          console.log('Owner changed successfully using set_value');
        } catch (setValueError) {
                  ...
                  <div className="flex items-center gap-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    ...
                    <div className="flex-1">
                      <p className="text-lg font-semibold text-gray-900">
                        {ownerData?.full_name || project?.owner || 'N/A'}
                      </p>
                      <p className="text-sm text-gray-600">
                        {ownerData?.email || project?.owner || 'No email'}
                      </p>
