...
export class UserAssignedTaskService {
  ...
  static useUserAssignedTasks(): UserAssignedTasksData {
    ...
    const { data: todos, isLoading: todosLoading, error: todosError, mutate: mutateTodos } = useFrappeGetDocList<ToDo>(
      'ToDo',
      {
        fields: [
          'name',
    ...
    const taskTodos = (todos || []).filter(todo => todo.reference_type === 'Task');
    const taskNames = taskTodos.map(todo => todo.reference_name).filter(Boolean) as string[];

    // Extract SubTask reference names
    const subTaskTodos = (todos || []).filter(todo => todo.reference_type === 'SubTask');
    const subTaskNames = subTaskTodos.map(todo => todo.reference_name).filter(Boolean) as string[];

    // Get Tasks details
    const { data: tasks, isLoading: tasksLoading, error: tasksError } = useFrappeGetDocList<Task>(
      'Task',
      {
        fields: [
          'name',
    ...
    const { data: subTasks, isLoading: subTasksLoading, error: subTasksError } = useFrappeGetDocList<SubTask>(
      'SubTask',
      {
        fields: [
          'name',
    ...
    const parentTaskNames = [...new Set((subTasks || []).map(st => st.task).filter(Boolean))] as string[];
    ...
    const { data: parentTasks } = useFrappeGetDocList<Task>(
      'Task',
      {
        ...
        filters: parentTaskNames.length > 0 ? [
    ...
    const projectNameMap = (projects || []).reduce((acc: Record<string, string>, project: any) => {
      acc[project.name] = project.project_name || project.name;
      return acc;
    }, {});

    // Create parent task mapping
    const parentTaskMap = (parentTasks || []).reduce((acc: Record<string, Task>, task: Task) => {
      acc[task.name] = task;
      return acc;
    }, {});

    // Create todo mapping
    const todoMap = (todos || []).reduce((acc: Record<string, ToDo>, todo: ToDo) => {
    ...
    const enrichedSubTasks: EnrichedSubTask[] = (subTasks || []).map(subTask => {
      const parentTask = subTask.task ? parentTaskMap[subTask.task] : undefined;
      return {
        ...subTask,
        task_subject: parentTask?.subject,
        project_name: parentTask?.project ? projectNameMap[parentTask.project] : undefined,
